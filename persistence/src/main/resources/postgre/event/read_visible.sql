...
(SELECT visible_events.event_id, c.friend_count, d.attendee_count, e.inviter_count
 FROM  
  ...
  LEFT OUTER JOIN
  (
    SELECT event_attendees.event_id, COUNT(*) AS friend_count
    FROM
      (
        SELECT
          user_id1 AS user_id,
  ...
  LEFT OUTER JOIN
  (
    SELECT event_attendees.event_id, COUNT(*) AS attendee_count
  ...
  LEFT OUTER JOIN
  (
    SELECT event_invitees.event_id, COUNT(*) AS inviter_count
    FROM event_invitees
    WHERE event_invitees.invitee_id = ?
